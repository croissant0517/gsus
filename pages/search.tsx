...
const SearchPage: NextPage = () => {
  ...
  useEffect(() => {
    if (!!router.query.keyword) {
      ...
      if (!!keyword && keyword !== '') {
        axios(`/api/get-search/${keyword}`).then((res) => {
    ...
    const scrollingFetch = () => {
      if (
        window.innerHeight + window.scrollY >=
          document.body.offsetHeight - 1080 &&
        !loading &&
        ...
        axios(`/api/get-search/${searchTerm}`, {
          ...
          const filtedVideosData = res.data.videos.filter(
            (video: Video) => video.width >= 1920
          );
      ...
      <div className={styles.pageContainer}>
        ...
        <div className={styles.resultContainer}>
          <div className={styles.resultAreaHeader}>
            <div>{`共 ${searchResults?.total_results ?? '0'} 部作品`}</div>
          </div>
